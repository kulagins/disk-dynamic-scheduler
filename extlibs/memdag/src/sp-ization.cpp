...
{ // Uses edges' generic_pointer
    ...
    while (!fifo_is_empty(sorted_vertices)) {
        ...
        } else if (current_vertex->in_edges.size() == 1) {
...
#endif
            for (int i = 0; i < current_vertex->out_edges.size(); i++) {
        ...
        if (current_vertex->out_edges.size() == 1) { // Update the (source,target) pair of the leaf node
#ifdef DEBUG_SP_IZATION
